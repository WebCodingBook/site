<li class="wow fadeInLeft" id="activity_{{ $activity->id }}">
    <time class="timeline-time" datetime="{{ $activity->date }}">{{ $activity->date }} <span class="hidden-xs">{{ $activity->month }}</span></time>
    @if( $activity->type != 'news' )
        <span class="timeline-2-avatar"><img src="{{ $activity->user->avatar() }}" alt="{{ $activity->user->full_name }}" class="img-circle img-responsive" title="{{ $activity->user->full_name }}"></span>
    @else
        <span class="timeline-2-point"></span>
    @endif
